    ...
    if faces_len == 0:
        ...
        while index < faces_len:
            ...
            if face_mask == THREEJS_TYPE_VERTEX_NORMAL:
                ...
                current_faces.extend(_map_triple(current_vertices, face_vertex_map, faces[index + 1: index + 4], vertices))
                current_faces.extend(_map_triple(current_normals, face_normal_map, faces[index + 4: index + 7], normals))
                index += mask_size
            else:
...
def _map_triple(current_values, value_map, source_triple, value_store, print_=False):
    """
    ...
    for source_value in source_triple:
        ...
        if source_value not in value_map:
            vertex = value_store[3*source_value:3*source_value + 3]

            current_values.extend(vertex)
            value_map[source_value] = int(len(current_values) / 3 - 1)
